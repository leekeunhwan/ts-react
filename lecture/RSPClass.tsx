...
class RSPClass extends Component<{}, IState> {
  state: IState = {
    result: "",
  ...
  componentDidMount() {
    // 컴포넌트가 첫 렌더링된 후, 여기에 비동기 요청을 많이 한다. (onload 시점)
    this.interval = window.setInterval(this.changeHand, 100);
  }

  componentWillUnmount() {
    // 컴포넌트가 제거되기 직전, 비동기 요청을 많이 한다. (이벤트 제거관련)
  ...
  onClickBtn = (choice: keyof typeof rspCoords) => (
    ...
    if (diff === 0) {
      ...
      this.setState(prevScore => {
        return { result: "이겼습니다!", score: prevScore.score + 1 };
      });
    } else {
      this.setState(prevScore => {
        return { result: "이겼습니다!", score: prevScore.score - 1 };
  ...
  render() {
    const { result, imgCoords, score } = this.state;
    return (
      <>
        <div
          id="computer"
